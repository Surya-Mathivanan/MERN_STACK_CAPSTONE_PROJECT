...
interface NavigationProps {
  ...
  onBack: () => void;
  userProfile?: { stage: string; level: string };
  onSignOut: () => void;
  user: any;
}
...
const Navigation: React.FC<NavigationProps> = ({
  ...
  return (
    <>
      {/* Desktop Navigation */}
      <nav className="hidden md:block bg-gradient-to-r from-space-dark to-space-blue border-b border-periwinkle/20 backdrop-blur-sm">
          ...
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center space-x-4">
              {showBackButton && (
                <button
                  onClick={onBack}
                  className="text-pale-pink/70 hover:text-white px-3 py-2 rounded-md text-sm font-medium transition-colors duration-200"
                >
                  ← Back
                </button>
              )}
