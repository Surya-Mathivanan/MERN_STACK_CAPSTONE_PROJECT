      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
          {filteredArticles.map((article, index) => (
            <div
              key={index}
              className="group bg-space-light/30 backdrop-blur-sm rounded-2xl p-6 border border-periwinkle/20 hover:border-dusty-rose/50 transition-all duration-300"
            >
              <div className="flex items-center justify-between mb-3">
                <span className={`text-xs font-semibold px-2 py-1 rounded-lg ${getSourceColor(article.source)}`}>
                  {article.source}
                </span>
